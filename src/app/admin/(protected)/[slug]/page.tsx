...
import { getTournamentBySlug, listPlayers, listRounds, listPairings } from "@/lib/db";
...
export default async function TournamentAdminDashboardPage({ params }: PageProps<"/admin/[slug]">) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat label="Rounds" value={`${completed} / ${tournament.roundsCount}`} />
        <Stat label="Time control" value={tournament.timeControl} />
      ...
      <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
                  ...
                  return (
                    <tr key={r.id} className="border-b border-slate-100 last:border-0">
                      ...
                      <td className="py-2.5 pr-4 text-right">
                        ...
                        <Link
                          href={`/admin/${tournament.slug}/rounds/${r.number}`}
        ...
        {!pendingRound && (
          <div className="border-t border-slate-100 px-4 py-3">
            <GenerateButton tournamentId={tournament.id} />
          </div>
        )}
      </section>
      ...
      {activePlayers.length < 2 && (
        <div className="rounded-lg border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800 ">
          ...
          <Link href={`/admin/${tournament.slug}/players`} className="font-medium underline">
            Manage players
          </Link>
        </div>
      )}
